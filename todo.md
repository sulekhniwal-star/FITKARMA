...
- [x] Create Appwrite Cloud account (or self-host) — note endpoint URL
- [x] Create RevenueCat account — note iOS & Android API keys
- [x] Create Sentry account — note DSN
- [x] Create Anthropic account — note API key (for AI Coach function)
...
- [x] Set `minSdkVersion 26` in `android/app/build.gradle`
- [x] Set `IPHONEOS_DEPLOYMENT_TARGET = 14.0` in Xcode
...
- [x] Add monetisation: `purchases_flutter` (RevenueCat)
...
- [x] Run `flutter pub get` and confirm no conflicts

### 1.3 Code Generation Bootstrap

- [x] Run `dart run build_runner build --delete-conflicting-outputs` (Success)
...
- [x] Connect fonts in `pubspec.yaml` and verify rendering on device (Connected in Phase 1.2)
...
- [x] Initialized function `fitkarma-core` using Node.js 22 runtime.
...
- [x] In Flutter, updated/implemented services to use `fitkarma-core`:
...
- [x] 7-day expiring share link — calls `fitkarma-core` with `action: 'generate_share_link'`
...
- [x] Search Indian cuisine terms (dal, roti, biryani, etc.)
- [x] Mark `"source": "edamam"`
- [x] Deduplicate against existing sources

#### Phase G — Spoonacular Recipe API (~5,000 Indian recipes)
- [x] Register at spoonacular.com/food-api (150 calls/day free)
- [x] Filter: `cuisine=indian`, convert per-serving to per-100g
- [x] Mark `"source": "spoonacular"`

#### Phase H — FAO/INFOODS (~2,000 Asian foods)
- [x] Download INFOODS South/East Asian Food Composition Tables
- [x] Extract Northeast Indian tribal foods (critical HealthifyMe gap)
- [x] Mark `"source": "fao_infoods"`
